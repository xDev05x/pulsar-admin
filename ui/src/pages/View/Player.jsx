...
export default ({ match }) => {
  ...
  const fetch = async (forced) => {
    if (!player || player.Source != match.params.id || forced) {
      ...
      try {
        let res = await (
          await Nui.send("GetPlayer", parseInt(match.params.id))
        ).json();
  ...
  const copyCoords = () => {
    Nui.copyClipboard(
      `vector3(${round(player.Character.Coords?.x, 3)}, ${round(
        player.Character.Coords?.y,
        3
      ...
      {loading || (!player && !err) ? (
        ...
        <>
          <Grid className={classes.wrapper} container spacing={2}>
              ...
              <ButtonGroup fullWidth variant="contained">
                ...
                <Button
                  ...
                  disabled={
                    !player?.Character ||
                    player.Disconnected ||
                    (user?.Source === player.Source && permissionLevel < 75)
                  }
                >
                  Heal
                </Button>
                ...
                <Button onClick={openForumUrl}>Copy Site URL</Button>
                ...
                <ListItem>
                  <ListItemButton onClick={onDisconnectedClick}>
                    <ListItemText
                      ...
                      secondary={`Disconnected ${moment(
                        player.DisconnectedTime * 1000
                      ).fromNow()}${
                        player.Reconnected
                          ? `, they have since reconnected to the server.`
                          : "."
            ...
            <Grid item xs={6}>
              <List>
                ...
                {player?.Vehicle && (
                  <ListItem onClick={viewVehicle}>
                    <ListItemText
                      primary="Inside Vehicle"
                      secondary={`${player?.Vehicle?.Make ?? "Unknown"} ${
                        player?.Vehicle?.Model ?? "Unknown"
                      }`}
                    />
                  </ListItem>
                )}
                ...
                {player?.Vehicle?.Driver && (
                  <ListItem onClick={viewVehicle}>
                    <ListItemText primary="Is Driver" secondary={"Yes"} />
                  </ListItem>
                )}
              </List>
            </Grid>
                  ...
                  <List>
                    ...
                    <ListItem>
                      <ListItemText
                        primary="DOB"
                        secondary={
                          player.Character.DOB
                            ? moment(player.Character.DOB).format("LL")
                            : "Unknown"
                        }
                      />
                    </ListItem>
                    ...
                    {permissionLevel >= 90 && (
                      <ListItem onClick={copyCoords}>
                        <ListItemText
                          primary="Coordinates"
                          secondary={`vector3(${round(
                            player.Character.Coords?.x,
                            3
                          )}, ${round(player.Character.Coords?.y, 3)}, ${round(
                            player.Character.Coords?.z,
                            3
                          )})`}
                        />
                      </ListItem>
                        ...
                        <Grid container>
                          ...
                          <Grid item xs={4}>
                            <ListItemText
                              primary="DOB"
                              secondary={
                                c.DOB ? moment(c.DOB).format("LL") : "Unknown"
                              }
                            />
                          </Grid>
                        </Grid>
          ...
          >
            ...
            >
              {banLengths
                .filter((l) => permissionLevel >= l.permissionLevel)
                .map((l) => (
